...
/** Текст и CSS-модификатор для бейджа статуса заказа. Все статусы в интерфейсе на русском. Оформлен = зелёный, выполнен = отдельный стиль, остальное — нейтрально. */
function getOrderStatusDisplay(order) {
    ...
    if (raw === 'completed') return { text: 'Заказ выполнен', modifier: '--done' };
    if (raw === 'in_progress' || raw === 'pending_production' || original.indexOf('на проверке') !== -1) return { text: 'На проверке у производства', modifier: '' };
    if (raw === 'cancelled' || raw === 'canceled' || raw === 'отмена') return { text: 'Отмена', modifier: '' };
...
function initEditOrderModal() {
    ...
    var completeBtn = document.getElementById('edit-order-complete-btn');
    if (completeBtn) {
        completeBtn.addEventListener('click', function () {
            ...
            supabaseClient.from('orders').update({ status: 'completed' }).eq('id', currentOrderIdForEdit).then(function (res) {
                if (res.error) throw res.error;
                if (typeof showToast === 'function') showToast('Заказ отмечен как выполненный.', 'success');
                if (lastEditOrderSearchedPhone) {
                    searchOrdersByPhone(lastEditOrderSearchedPhone).then(function (orders) {
                        renderEditOrderList(orders);
                        var searchHint = document.getElementById('edit-order-search-hint');
