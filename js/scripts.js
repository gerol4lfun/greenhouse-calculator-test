...
/** Текст и CSS-модификатор для бейджа статуса заказа (оформлен = зелёный, остальное — нейтрально). Все статусы показываем на русском. */
function getOrderStatusDisplay(order) {
    var raw = (order.status || '').trim().toLowerCase();
    if (raw === 'synced' || raw === 'confirmed' || !raw) return { text: 'Оформлен', modifier: '--ok' };
    if (raw === 'new') return { text: 'Новый', modifier: '' };
    if (raw === 'cancelled' || raw === 'canceled') return { text: 'Отменён', modifier: '' };
    if (raw === 'duplicate' || raw === 'дубль') return { text: 'Дубль', modifier: '' };
    if (raw === 'error') return { text: 'Ошибка', modifier: '' };
    return { text: (order.status || '').trim() || '—', modifier: '' };
}
...
function validateEditOrderModal() {
    ...
    if (!isValidPhone11(normalizedPhone)) errors.push('телефон: 11 цифр, с 7');
...
async function submitOrder() {
    ...
    try {
        ...
        resultDiv.innerHTML = '<span class="order-result-message">✅ Заказ оформлен!</span><button type="button" class="order-result-edit-btn" tabindex="0">Изменить заказ</button>';
        resultDiv.className = 'success';
        resultDiv.style.display = '';
