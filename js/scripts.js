...
async function initializeCalculator() {
    ...
    // Deep link: ?editPhone=79001234567 → открыть модалку редактирования нужного заказа
    ...
    if (editPhoneParam) {
        setTimeout(function () {
            ...
            if (phoneInput) {
                phoneInput.value = editPhoneParam;
                var evt = new Event('input', { bubbles: true });
                phoneInput.dispatchEvent(evt);
            }
            if (typeof openEditOrderModal === 'function') openEditOrderModal();
        }, 400);
    }
}
