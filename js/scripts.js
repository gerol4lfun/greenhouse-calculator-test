...
async function initializeCalculator() {
    ...
    // Deep link: ?editPhone=79001234567 → модалку уже открыл inline в HTML; здесь только запуск поиска
    ...
    if (editPhoneParam) {
        ...
        var alreadyOpen = modal && !modal.classList.contains('hidden');
        if (!alreadyOpen && typeof openEditOrderModal === 'function') openEditOrderModal();
        var phoneInput = document.getElementById('edit-order-phone');
        if (phoneInput && !phoneInput.value) phoneInput.value = editPhoneParam;
        var searchBtn = document.getElementById('edit-order-search-btn');
        if (searchBtn && typeof normalizePhone === 'function' && normalizePhone(editPhoneParam).length === 11) setTimeout(function () { searchBtn.click(); }, 0);
    }
}
