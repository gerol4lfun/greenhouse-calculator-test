// @ts-check
/**
 * LIVE staging: native existing order date-only edit (real PATCH, no intercept).
  ...
  if (await auth.isVisible().catch(() => false)) {
    await page.evaluate(
      ({ login, password }) => {
        ...
        const btn = document.querySelector('#auth-container button');
        if (btn) btn.click();
      },
      { login, password }
    );
...
test('staging native: existing order date-only edit keeps delivery/total', async ({ page }) => {
  ...
  const orderCandidates = (process.env.STAGING_NATIVE_DATE_ORDER_IDS ||
    '671d847f-fdb4-460f-b262-50f7c1a03f8c')
    .split(',')
    .map((s) => String(s || '').trim())
    .filter(Boolean);
  ...
  const facts = {
    ...
    scenario: 'native-date-only-live',
  ...
  if (!orderId || !before || !beforeMoney || !afterPick) {
    test.skip(true, 'Native date-only: no alternate available date found on candidate orders.');
  }
  ...
  const outPath = path.join(__dirname, '.last-staging-native-date-only-live.json');
